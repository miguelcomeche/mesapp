...
export default function RestaurantUsersPage() {
  ...
  const load = async () => {
    ...
    // Permission check
    const isPlatform = hasRole('platform_admin');
    let isRestAdmin = false;
    if (!isPlatform) {
      const { data } = await supabase.rpc('has_restaurant_role' as any, {
        _user: user.id, _restaurant: restaurantId, _role: 'restaurant_admin',
      } as any);
      isRestAdmin = !!data;
    }
    if (!isPlatform && !isRestAdmin) {
      setAllowed(false);
      setLoading(false);
      return;
    }
    ...
    setRestaurantName((r as any)?.name ?? '');
  ...
  useEffect(() => { load(); /* eslint-disable-next-line */ }, [restaurantId, user?.id]);
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Button size="sm" variant="ghost" onClick={() => navigate('/admin/restaurants')}>
                ...
                <TableRow key={m.user_id}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-1">
                      <Button size="sm" variant="ghost" onClick={() => { setEditing(m); setFormOpen(true); }} title="Editar rol">
